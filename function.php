...
function getrecepteur($id = null)
{
    if (!empty($id)) {
        $sql = "SELECT * FROM recepteur WHERE id=?";
...
{
    if (!empty($id)) {
        $sql = "SELECT nom_article, nom, prenom, v.quantite, prix, date_sortie, v.id, prix_unitaire, adresse, telephone
        FROM recepteur AS c, sortie AS v, article AS a WHERE v.id_article=a.id AND v.id_recepteur=c.id AND v.id=? AND etat=?";
        ...
        $sql = "SELECT nom_article, nom, prenom, v.quantite, prix, date_sortie, v.id, a.id AS idArticle
        FROM recepteur AS c, sortie AS v, article AS a WHERE v.id_article=a.id AND v.id_recepteur=c.id AND etat=?";
...
{
    $sql = "SELECT COUNT(*) AS nbre FROM sortie WHERE etat=?";
...
function getCA()
{
    $sql = "SELECT SUM(prix) AS prix FROM sortie";
